...
class categories extends CI_Controller {
	...
	function items_list()
	{
		...
		elseif($res->role_id == 1):
			...
			$out['is_logged'] = true;
			$res = $this->mReference->all('categories');
			if(!empty($res)):
				foreach($res as $row):
					$out['lists'][] = (array)$row;
				endforeach;
	...
	function item_save()
	{
		...
		elseif($res->role_id == 1):
			...
			$res = $this->mReference->get('categories', $id);
			if (empty($res)):
				$res = $this->mReference->insert('categories', $arr);
			else:
				$res = $this->mReference->update('categories', $arr, array('id' => $id));
			endif;
			if ($res > 0):
	...
	function item_delete()
	{
		...
		elseif($res->role_id == 1):
			$out['is_logged'] = true;
			$id = (int)$json['id'];
			$cnt = $this->mReference->cnt_where('software', array('category_id' => $id));
			if ($cnt > 0):
				$out['message'] = 'Нельзя удалить элемент, на который имеются ссылки!';
			else:
				$res = $this->mReference->delete('categories', array('id' => $id));
				if ($res > 0):
